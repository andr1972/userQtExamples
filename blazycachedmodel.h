 ...
 *
 */
class BLazyCachedModel : public QAbstractItemModel
{
  Q_OBJECT
    struct TreeNode {
      TreeNode *parent;
      ...
      int index;
      bool opened = false;
      TreeNode(TreeNode *p, int level, int index) : parent(p), level(level), index(index) {}
      ~TreeNode() {}
...
private:
  ...
  int usualSize;
public:
  explicit BLazyCachedModel(QObject *parent, int maxLevel, int usualSize);
  virtual ~BLazyCachedModel() override;
